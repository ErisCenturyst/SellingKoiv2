...
<<<<<<< HEAD
            try
            {
                ...
                if (!farmExists)
                {
                    throw new ArgumentException("Invalid FarmID.");
                }

                var existingKoi = await _context.KOIs.FindAsync(koi.Id);
                if (existingKoi == null)
                {
                    throw new ArgumentException("Koi not found.");
                }

                existingKoi.Name = koi.Name;
                existingKoi.Type = koi.Type;
                existingKoi.Age = koi.Age;
                existingKoi.Description = koi.Description;
                existingKoi.FarmID = koi.FarmID;
